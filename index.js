(function () {
  ...
  function loadData() {
    let rawData = []
    const dataFollowing = JSON.parse(localStorage.getItem('followingContacts')) || []
    axios
      .get(INDEX_URL)
      .then((response) => {
        rawData = response.data.results

        // 讀入資料庫後，也檢查local storage的following清單中是否已含此item，如果有就將 follow 設為 true
        rawData.forEach(function (item) {
          const result = dataFollowing.find(data => Number(data.id) === Number(item.id))
          if (result) {
            data.push({ ...item, follow: true })
          } else {
            data.push({ ...item, follow: false })
          }
        })
        // 每頁陳列12筆資料，計算所需頁數並取得該頁數之data
        getTotalPages(data)
        displayMode = 'card'
        getPageData(1, data)
        console.log(data)
  ...
  function getPageData(pageNum, data) {
    ...
    // deactive all pagination state
    Array.prototype.forEach.call(pagination.children, item => {
      item.className = "page-item"
    })
    // set the current page state to active
      ...
      data.forEach(function (item, index) {
        htmlContent += `
            <tr class="row ml-3 mr-3">
              <td class="col-2 col-sm-1"><img src="${item.avatar}" style="width: 40px; border-radius: 50%;"></td>
              <td class="col-5 col-sm-6">${item.name} ${item.surname}</td>
